...
const DressStyleList: React.FC<DressStyleListProps> = ({
    dressStyleList,
    onChange,
}) => {
    ...
    useEffect(() => {
        setStyles(dressStyleList);
    }, [dressStyleList]);

    const styleHandler = (style: string) => {
        setStyles(
            styles.map((s: DressStyleProps) => {
                if (s.style === style) {
                    return {
                        ...s,
                        checked: !s.checked,
        ...
        <div className="flex flex-col gap-4 h-35">
            {styles.map((dressStyle, index) => {
                ...
                return (
                    <div
                        key={`style_${index}`}
                        className="flex flex-row items-center hover:cursor-pointer justify-between"
                        onClick={() => styleHandler(dressStyle.style)}
                    >
