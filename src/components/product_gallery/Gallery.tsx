...
const Gallery: React.FC<GalleryProps> = ({
    ...
    return (
        <div className="flex flex-col flex-wrap items-center justify-center py-4 pb-8 border-b-2 border-gray-200">
            <h1 className="text-center text-3xl font-header uppercase p-8">
                {headerTitle}
            </h1>
            {products != null &&
                products != undefined &&
                products.items.length > 0 && (
                    <div
                        ref={scrollRef}
                        className="flex gap-4 w-full p-6 overflow-x-auto no-scrollbar"
                    >
                        {products.items
                            .slice(sliceArr[0], sliceArr[1])
                            .map((item, index: number) => {
                                return (
                                    <div
                                        key={`new_arrival_${index}`}
                                        className="min-w-[250px] w-1/4 flex-shrink-0"
                                    >
