...
const GalleryItem: React.FC<Product> = (product) => {
    return (
        <div className="bg-white rounded-2xl shadow-md p-4">
            <Link to={`/Shop/${product.categories[0].name}/${product.id}`} state={{ categoryId: product.categories[0].id }}>
                <img src={product.ProductImgs[0].url} alt={product.ProductImgs[0].file_name} className='w-full object-cover' />
            </Link>
            <div className='p-4'>
